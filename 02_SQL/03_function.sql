...
- 숫자관련 함수
 abs(값): 절대값 반환
 round(값, 자릿수): 자릿수이하에서 반올림 (양수 - 실수부(0부터 -n), 음수 - 정수부(1부터 n), 기본값: 0-0이하에서 반올림이므로 정수로 반올림)
...
SELECT subdate(CURDATE(),interval 10 month),
        ...
        adddate(now(), interval 10 hour);
-- 날짜/시간의 차이 계산
select datediff(curdate(),'2025-10-10'); -- 2025-10-20 - 2025-10-10의 결과

SELECT timediff(curtime(),'10:10:20');

select timestampdiff(quarter, '2024-10-10', now());
...
select subdate(hire_date, interval 10 day), 
		hire_date,
...
# y 소문자는 yy, 대문자 Y 는 yyyy, 대문자 M은 영문표기 소문자 m은 숫자, 대문자 D도 영문, 소문자 d는 숫자만, 소문자 h는 12 시간표기, 대문자 H는 24시
from emp
where emp_id = '200';
...
select emp_id, emp_name, job, ifnull(dept_name,'  배치 전  ')
from emp
where dept_name is null;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션 (salary * comm_pct)을 조회. 커미션이 없는 직원은 0을 출력.
select emp_id, emp_name, salary, ifnull(salary*comm_pct, 0) '커미션'
from emp;
...
select emp_id, emp_name, job,
	case job when 'AD_PRES' then '대표'
            ...
            else job # 출력값에 column을 작성하면 해당 column 에 위치한 행의 본래 값이 출력됨
            end "job"
from emp
