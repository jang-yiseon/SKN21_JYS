...
- 2개 이상의 테이블에 있는 컬럼들을 합쳐서 가상의 테이블을 만들어 조회하는 방식을 말한다.
 	- 소스테이블 : 내가 먼저 읽어야 한다고 생각하는 테이블 → main 정보
	- 타겟테이블 : 소스를 읽은 후 소스에 조인할 대상이 되는 테이블 → target 정보
...
- 조인의 종류
    ...
    - Cross Join
        - 두 테이블의 곱집합을 반환한다. 
        - A table의 A-1을 B table의 각 값들(B-1,B-2,B-3...)과 하나씩 결합하는 것.
******************************************************************************** */        
USE HR_JOIN;
...
# 3개의 table이 각각 있어 조회를 따로따로해야함.
# 이 번거로움을 없애기위해 이 3개의 table로 하나로 만든 table을 만듦(join)
# from절에서 join을 처리함
...
select d.dept_id, d.loc, e.emp_name
...
select e.emp_id, e.emp_name "직원 이름", m.emp_name "상사 이름"
...
select d.dept_id, d.dept_name, count(e.emp_id) "직원 수" ## 내가 count하려는 table의 pk로 count를 지정하는게 좋음
...
select e.emp_id, e.emp_name, m.emp_name, date_format(e.hire_date,'%Y/%m/%d') "입사일"
from emp e join emp m on e.mgr_id = m.emp_id
where e.dept_id=90;
...
select e.emp_id, d.*, j.job_title, m.emp_name "상사이름" ## d.* → 해당 table에 있는 column을 다 보겠다는 뜻. 
from emp e  left join job j on e.job_id = j.job_id
            left join dept d on e.dept_id = d.dept_id
			left join emp m on e.mgr_id = m.emp_id
where year(e.hire_date) between 2003 and 2005;
