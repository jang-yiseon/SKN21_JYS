...
# sub query는 무조건 ()를 통해 묶어놔야함. 생략안됨 
...
select dept_id, dept_name, concat('$', format(급여평균,2)) "급여 평균"
# 이렇게 작성한 이유는 select문에서 concat과 format을 사용 후 정렬하면 9-1 다음에 10이 나오는 현상때문에 그것을 방지하고자 이렇게 작성
from (
	select d.dept_id, d.dept_name, avg(salary) "급여평균"
...
from detp
where detp_id = (select dept_id from emp where salary = (select max(salary) from emp));
...
select * from emp
where (job_id != 'ST_CLERK' or job_id is null) and salary < (select avg(salary) from emp where job_id = 'ST_CLERK');
...
## 결과가 한행으로 나온것을 단일 행 sub query라고 함.
...
select salary from emp where dept_id = 20 order by 1 desc;
