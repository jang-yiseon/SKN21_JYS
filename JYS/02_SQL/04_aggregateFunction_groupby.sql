...
select count(distinct(ifnull(dept_name,'a'))) from emp;
--  커미션 비율(comm_pct)이 있는 직원의 수를 조회
select count(comm_pct) from  emp;

--  최고 급여액과 최저 급여액 그리고 그 둘의 차액을 출력
select min(salary), max(salary), max(salary)-min(salary) from emp;
...
-- comm_pct 평균
select avg(comm_pct),# comm_pct가 있는 35명 기준 평균
avg(ifnull(comm_pct,0)) # 전체 직원에 대한 평균, 0도 숫자로 인식
from emp;
...
select  case when salary <= 10000 then '10,000이하'
		else '10,000초과' end "급여 기준", count(*)
from emp
group by 
	case when salary <= 10000 then '10,000이하'
		else '10,000초과'
		end;

-- 부서명(dept_name), 업무(job)별 직원수, 최고급여(salary)를 조회. 부서이름으로 오름차순 정렬.
select dept_name, count(*), max(salary)
from emp
group by dept_name, job;
...
select year(hire_date), avg(salary), count(*)
from emp
group by year(hire_date)
having count(*) >=20;
...
select avg(salary) "평균급여" , year(hire_date) "입사년도"
from emp
group by year(hire_date)
