...
select salary, salary * 10, salary / 10 from emp;

-- EMP 테이블에서 직원의 이름(emp_name), 급여(salary), 급여(salary)을 연봉으로 조회. (곱하기 12)
select emp_name, salary, salary * 12 "연봉" from emp;
...
select emp_name, salary, comm_pct, salary * comm_pct
from emp; 

select salary * comm_pct is null
from emp;
...
where emp_name like '%!%%' escape'&';
		-- escape '특수문자' → 특수문자 뒤에 %나 _는 조회할 값임을 표시(like의 패턴문자가 아님) 
